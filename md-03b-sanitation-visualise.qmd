...
```{r}
sani_USA_2000_2020 <- sanitation |>
  filter(iso3 == "USA", varname_short !="san_sm", year %in% c(2000, 2020))
```
...
```{r}
ggplot(data = sani_USA_2000_2020,
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    They go from highest service level to lowest.

2.  What would you want to change?

    In the case of the United States it is nearly impossible to see the other categories.

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    This category would have overwhelmed the data set, and the bars would have been impossible to read other categories. At least for the USA.
...
2.  What do you observe when you look at the code and plot?
    ...
    These plots are histograms of the frequency of countries with the four categorical levels divided up by national, rural, and urban groupings.
